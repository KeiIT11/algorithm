...
# サンプル値の座標を乱数で初期化
p = np.random.randint(0, 100, (num_point, 3)).tolist()

c1 = np.random.randint(0, 100, 3).tolist()  # クラスタ1の中央の座標
c2 = np.random.randint(0, 100, 3).tolist()

count = [0]  # 試行回数をカウントする用の変数
z_new = np.array([0 for x in range(num_point)]).tolist()  # 各点の状態を保存。０ならc1に近い、１ならc2に近い
z_past = np.array([2 for x in range(num_point)]).tolist()  # 過去の点の状態を記憶する箱(初期値は2、比較用）
...
def k_means():
    ...
    while np.any(z_new != z_past):
        c1_data.append(c1[:])  # Cのデータを一覧に格納
        c2_data.append(c2[:])  # Cのデータを一覧に格納
